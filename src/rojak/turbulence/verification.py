...
class DiagnosticAmdarVerification:
    ...
    # def closest_data_point(
    #     self, validation_conditions: "list[DiagnosticValidationCondition]", turbulence_diagnostic: "xr.DataArray"
    # ) -> xr.Dataset:
    #     assert len(set(validation_conditions)) == len(validation_conditions), (
    #         "Validation conditions must be unique to ensure name of dd.Aggregation is unique to prevent
    #         data corruption"
    #     )
    #     assert {"pressure_level", "longitude", "latitude", "time"}.issubset(turbulence_diagnostic.coords)
    #     space_time_columns: list[str] = [
    #         self._data_harmoniser.common_time_column_name,
    ...
    #         condition.observed_turbulence_column_name for condition in validation_conditions
    #     ]
    #     target_data: "dd.DataFrame" = self.data[space_time_columns.extend(validation_columns)]
    #     target_data = target_data.map_partitions(
    #         lambda df: df.assign(
    #             lat_index=map_values_to_nearest_coordinate_index(df.latitude,
    #             turbulence_diagnostic["latitude"].values),
    #             lon_index=map_values_to_nearest_coordinate_index(
    #                 df.longitude, turbulence_diagnostic["longitude"].values
    #             ),
    #             # level_index
    #         )
    #     )
    #     group_by_columns: list[str] = [self._data_harmoniser.common_time_column_name, "lat_index", "lon_index",
    #     "level"]
    #     grouped_by_space_time = target_data[group_by_columns.extend(validation_columns)].groupby(group_by_columns)
    #
    #     # whether any observations were made
    #     observations: xr.Dataset = xr.Dataset(
    #         data_vars={
    #             condition.observed_turbulence_column_name: xr.full_like(turbulence_diagnostic, np.nan, dtype="bool")
    #             for condition in validation_conditions
    #         }
    #     )
    #
    ...
    #     }
    #     # whether any observations were made
    #     was_turbulence_observed: "dd.DataFrame" = grouped_by_space_time.aggregate(aggregation_spec)
    #
    #     # for row in was_turbulence_observed.itertuples():
    #     #     row_multi_index = row.Index
    #     #     positional_indexer: dict = {
    #     #         "time": row_multi_index[0],
    #     #         "longitude": row_multi_index[2],
    #     #         "latitude": row_multi_index[3],
    #     #     }
    #     #     for condition in validation_conditions:
    #     #         observations[condition.observed_turbulence_column_name].loc[dict(level=row_multi_index[1])][
    #     #             positional_indexer
    #     #         ] = row[condition.observed_turbulence_column_name]
    #
    #     return observations
    #
    # def execute(
    ...
    #
    #     # whether any observations were made
    #     # was_turbulence_observed = xr.full_like(turbulence_diagnostic, np.nan, dtype="bool")
    #
    #     grouped_by_space_time = self.data[group_by_columns.extend(validation_columns)].groupby(group_by_columns)
    #
    #     aggregation_spec: dict = {
    #         condition.observed_turbulence_column_name: _observed_turbulence_aggregation(condition)
    #         for condition in validation_conditions
